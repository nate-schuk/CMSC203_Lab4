 ...
 * Description: Manipulates multidimensional arrays and performs different methods on the matrix. 
 * Due: 10/31/2024
	  ...
	  for(int row = 0; row < intArray.length; row++) {
		  for(int col = 0; col < intArray[row].length; col++) {
			  if(intArray[row][col] == value)
				  found++;
			  animate(row, col, found);
		  }
	  }
	  
      return found; // replace this line with your return statement
   }
